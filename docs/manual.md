...
数据每分钟直接从本机采样日志重新汇总，不再读取旧 HTML 快照。跨午夜、标签页重新可见或电脑休眠恢复后会重新检查；“今天”跟随日历，主动选中的历史日期保持不变。不重载页面、不打断音乐、不清空正在填写的活动。后台每五分钟生成的 HTML 只是离线副本，不决定在线页面的日期。空闲不等同摸鱼，电脑活动不等同科研产出；日均只计算有实际活动的日期。
...
页面顶上只有一块画板：品牌行、标题和音乐行都住在里面，共用同一幅画。画板按画的比例（3:1）长高，整幅雨林峡谷——两侧的苔藓崖壁、深处的雾、从谷口淌出来的河——都在框里，不会被裁掉上半截。音乐行没有自己的卡片底板，也不在画上铺任何蒙版，字直接浮在林子上——每个字描一圈页面底色的光，叶子和树干就从笔画边缘退开；标题下方留白，是留给后续音乐播放器的位置。个性签名默认为空，写了才出现在标题下面。

画上叠着三层差速雨丝、随阵风倾斜的雨幕、叶尖滴水、湿地涟漪与穿过林冠的光束，还有会缓慢歪头的木灵和随专注时长增减的星。三位神灵按天气现身，各有自己的落脚地，绝不换地方：晴天的鹿在过河：站在正对播放键和循环键下方的那道浅滩里，小腿化进水面，两处落脚点每次重新出现换一边；画板不够高、身子会被控件挡掉大半时，它改回河的左岸或右岸站着、大雾里的白狼卧在右侧大树脚下的苔藓坡、暴雨中的山兽神立在峡谷最深处的河湾。每位大约有一到两成的天气周期是完全显形的，身量跟着画板一起长，屏幕越大越壮观。镜头只跟版面走：天气变化不会让画面横移；窄屏裁到谁的家不在画面里，谁就不出场。标题仍保持单行。

插画由 AI 生成、压缩为本地 WebP，位于 `smallgalaxy/assets/art/`；不是影片截图。`dashboard-decor.js` 用 SVG/CSS 绘制动态层。图片随 HTML 内嵌，页面加载时不取外部资源。深浅主题各自适配；“个性化 → 森林与雨林背景”可整体关闭，“装饰画浓度”（0–150%）调整可见程度，0% 隐藏画面。关闭动画或系统开启“减少动态效果”时保留静态插画、停用粒子和天气计时器；标签页隐藏时暂停动画，装饰不参与统计。

### 天气循环

循环演绎晨间湿雾 → 林冠渐暖 → 午后积云 → 阵风骤雨 → 雨势渐歇 → 叶尖滴水、水汽回升 → 雨后透光 → 暮色归林。云先于雨到来；停雨后叶尖仍滴水，地面不会瞬间变干。参考[新加坡气象局的热带气候说明](https://www.weather.gov.sg/climate-climate-of-singapore/)，是压缩时间的氛围模拟，不是实时天气，也不代表东南亚每个地区、季节每天都如此。一轮默认 96 秒，可调到 20 秒–10 分钟，也能固定为林间细雨、热带骤雨或雨后晴光。没有闪电频闪或自动雷声音效。

天气模型独立插值雨量、光照、云层、雾、风、湿润度与树冠滴水量，再映射到 CSS 变量。近处雨滴更快、更长，远处更细、更慢；粒子持续运行，不因切换雨量而重新开始。减弱动态效果和关闭装饰时计时器停止。
...
`summary.py` 和 `dashboard.py` 共用统计逻辑。本机 HTTP 服务在打开页面和请求 `/data` 时直接重新汇总日志，浏览器每分钟局部更新，并在跨天、重新可见时检查更新，不使用 `<meta refresh>`。即使采样进程停止，也能显示新一天的空记录和真实记录状态；不会拿昨天的汇总充当今天。`#today` 随日历前进，`#YYYY-MM-DD` 表示主动选中的历史日期。静态 HTML 是离线副本，直接以 `file://` 打开不能自动取数。
